...
(function () {
  const { escaparHtml, header, footer, ligarNavegacaoInferior, aviso, carregando, spinner, svgLupa } = PV.ui;
  ...
  async function contato(main, ctx) {
    ...
    main.innerHTML = `${header()}<div class="pv-carregando" style="min-height:200px">${spinner()}</div>${footer(ctx.usuario.tipo)}`;

    let dados;
    try {
      dados = await PV.db.contatos.buscar(tipo);
    } catch (e) {
      main.innerHTML = `${header()}<div class="tela-contato">${aviso({ tipo: 'erro', texto: e.message || 'Não foi possível carregar este contato.' })}</div>${footer(ctx.usuario.tipo)}`;
